...
def create(content, secret):
    ...
    return len(content).to_bytes(4, 'big') + secret.to_bytes(4, 'big') + \
           (1).to_bytes(2, 'big') + (40).to_bytes(2, 'big') + content + bytes(pad)


def check_header(content, secret):
    pre = int.from_bytes(content[4:8], 'big')
    step = int.from_bytes(content[8:10], 'big')
    digit = int.from_bytes(content[10:12], 'big')
    if pre != secret or step != 2 or digit != 40:
